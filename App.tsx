...
export default function App() {
  ...
  const downloadCSV = () => {
    ...
    if (activeMode === 'Table') {
      csvContent += "Timecode,Description,Objects\n";
      timecodeList.forEach(row => {
        const objects = row.objects ? `"${row.objects.join(', ')}"` : "";
        csvContent += `${row.time},"${row.text.replace(/"/g, '""')}",${objects}\n`;
      });
    } else if (activeMode === 'Chart') {
      csvContent += `Timecode,${chartLabel || 'Value'}\n`;
      timecodeList.forEach(row => {
        csvContent += `${row.time},${row.value}\n`;
      });
    } else {
      csvContent += "Timecode,Content\n";
      timecodeList.forEach(row => {
        csvContent += `${row.time},"${row.text.replace(/"/g, '""')}"\n`;
      });
    }
    ...
    link.setAttribute("download", `analysis_${activeMode}_${new Date().getTime()}.csv`);
          ...
          {isLoading ? (
            ...
            <>
              <div className="outputHeader">
                <button className="button downloadButton" onClick={downloadCSV}>
                  <span className="icon">download</span> Download CSV
                </button>
              </div>
              {activeMode === 'Table' ? (
                <table>
